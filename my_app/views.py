...
@login_required
def reelbest_add_comment(request, reel_id):
    if request.method != 'POST':
        return JsonResponse({'error': 'POST talab qilinadi'}, status=405)
...
@login_required
def reelbest_view_increment(request, reel_id):
    if request.method != 'POST':
        return JsonResponse({'error': 'POST talab qilinadi'}, status=405)
    ReelBest.objects.filter(id=reel_id).update(views_count=F('views_count') + 1)
